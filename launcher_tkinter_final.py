...
class MinecraftLauncher:
    def __init__(self, root):
        ...
        ttk.Label(version_frame, text="Game Version:").pack(side=tk.LEFT)
        ...
        # Java检测按钮
        self.java_button = ttk.Button(button_frame, text="Check Java", command=self.check_java)
        self.java_button.pack(side=tk.LEFT, padx=5)

        # 启动按钮
        self.launch_button = ttk.Button(button_frame, text="Launch Game", command=self.launch_game)
    ...
    def load_versions(self):
        ...
        self.status_var.set("Version list loaded")
    ...
    def launch_game(self):
        ...
        self.progress.configure(value=0)
        self.status_var.set("Preparing to launch game...")
    ...
    def _launch_game_thread(self, username, version):
        ...
        try:
            ...
            # 更新状态
            self.root.after(0, lambda: self.status_var.set("Checking Minecraft installation..."))
            ...
            # 更新状态
            self.root.after(0, lambda: self.status_var.set("Checking game version..."))
            ...
            if not os.path.exists(version_file):
                # 下载版本
                self.root.after(0, lambda: self.status_var.set(f"Downloading Minecraft {version}..."))
                self.progress.configure(value=30)
                if not self._download_version(mc_path, version):
                    self.root.after(0, lambda: messagebox.showerror("Error", f"Failed to download Minecraft {version}"))
                    return

            # 更新状态
            self.root.after(0, lambda: self.status_var.set("Preparing to launch game..."))
            self.progress.configure(value=70)

            # 检查jar文件是否存在
            jar_file = os.path.join(version_dir, f"{version}.jar")
            if not os.path.exists(jar_file):
                self.root.after(0, lambda: messagebox.showerror("Error", f"Game file not found: {jar_file}"))
                return

            # 构建启动命令
            self.root.after(0, lambda: self.status_var.set("Launching game..."))
            ...
            error_msg = str(e)
            self.root.after(0, lambda msg=error_msg: messagebox.showerror("错误", f"启动游戏时出错: {msg}"))
        finally:
            # 恢复按钮状态
    ...
    def check_java(self):
        """检测Java环境，如果没有则自动安装"""
        self.status_var.set("正在检测Java环境...")
        self.progress.configure(value=10)

        # 在新线程中执行检测操作，避免界面冻结
        thread = threading.Thread(target=self._check_java_thread)
        thread.daemon = True
        thread.start()

    def _check_java_thread(self):
        """在新线程中执行Java检测操作"""
        try:
            # 检查Java是否已安装
            java_path = self._find_java()
            ...
            if java_path:
                # Java已安装
                self.root.after(0, lambda: self.status_var.set("Java环境已安装"))
                self.root.after(0, lambda: self.progress.configure(value=100))
                self.root.after(0, lambda: messagebox.showinfo("成功", f"Java环境已安装!\n路径: {java_path}"))
            else:
                # Java未安装，尝试自动下载
                self.root.after(0, lambda: self.status_var.set("未找到Java，正在自动下载..."))
                java_path = self._download_java()

                if java_path:
                    self.root.after(0, lambda: self.status_var.set("Java环境安装成功"))
                    self.root.after(0, lambda: self.progress.configure(value=100))
                    self.root.after(0, lambda: messagebox.showinfo("成功", f"Java环境安装成功!\n路径: {java_path}"))
                else:
                    self.root.after(0, lambda: self.status_var.set("Java环境安装失败"))
                    self.root.after(0, lambda: self.progress.configure(value=0))

        except Exception as e:
            error_msg = str(e)
            self.root.after(0, lambda msg=error_msg: messagebox.showerror("错误", f"检测Java环境时出错: {msg}"))
            self.root.after(0, lambda: self.progress.configure(value=0))
            self.root.after(0, lambda: self.status_var.set("就绪"))
    ...
    def _download_java(self):
        ...
        try:
            ...
            error_msg = str(e)
            self.root.after(0, lambda msg=error_msg: messagebox.showerror("错误", f"下载Java运行环境时出错: {msg}"))
            return None
